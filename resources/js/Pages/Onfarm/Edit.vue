  ...
  <div>
    ...
    <Auth>
      <template #header>
        ...
        <InertiaLink
          class="text-primary"
          :href="route('field.planting.index', onfarm.planting.field_id)"
          >Penanaman</InertiaLink
        >
        <i class="fa fa-chevron-right mx-2"></i>
        <InertiaLink
          class="text-primary"
          :href="route('field.planting.show', [onfarm.planting.field_id, onfarm.planting_id])"
      ...
      <div class="row justify-content-center">
        <div class="col-md-8">
          <div class="card card-body p-4">
            ...
            <div>
              <div>Lahan #{{ onfarm.planting.field_id }}</div>
              <div>Lokasi: {{ onfarm.planting.field.address.formatted_address }}</div>
              <div>Pemilik: {{ onfarm.planting.field.user.name }}</div>
              <div>
                Tanggal tanam:
                {{
                  format(new Date(onfarm.planting.started_at), "d MMMM yyyy", {
                    locale: id,
                  })
                }}
              </div>
...
export default {
  ...
  data() {
    return {
      form: this.$inertia.form({
        name: this.onfarm.name,
        category: this.onfarm.category,
        description: this.onfarm.description,
        started_at: this.onfarm.started_at,
        finished_at: this.onfarm.started_at,
        costs: this.onfarm.costs,
      }),
  ...
  methods: {
    submit() {
      this.form.put(route("planting.onfarm.update", [this.onfarm.planting_id, this.onfarm.id]));
    },
    addCost() {
